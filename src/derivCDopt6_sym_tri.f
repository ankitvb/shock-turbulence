...
*  alpha =   5.381301488732363  E-01
      ...
      Do N = 1, LotX  ! (My * Mz)
        Do I = 1,Mx
          ldu_x_sym_A1(N,I) = alpha1_pade6
          ldu_x_sym_B1(N,I) = 1.0D0
          ldu_x_sym_C1(N,I) = alpha1_pade6
        End Do
      End Do

      Do N = 1, LotY  ! (Mz * Mx)
        Do J = 1,My
          ldu_y_sym_A1(N,J) = alpha1_pade6
          ldu_y_sym_B1(N,J) = 1.0D0
          ldu_y_sym_C1(N,J) = alpha1_pade6
        End Do
      End Do

      Do N = 1, LotZ  ! (Mx * My)
        Do K = 1,Mz
          ldu_z_sym_A1(N,K) = alpha1_pade6
          ldu_z_sym_B1(N,K) = 1.0D0
          ldu_z_sym_C1(N,K) = alpha1_pade6
        End Do
      End Do
...
*                      h     2                   2
      ...
      if(xrank .EQ. 0)then
       Do N = 1,LotX
        ...
        ldu_x_sym_B1(N,1) = 1.0D0
        ldu_x_sym_C1(N,1) = 0.0D0
       End Do
      endif

      if(xrank .EQ. Px-1)then
       Do N = 1,LotX
        ldu_x_sym_A1(N,Mx) = 2.0D0
        ldu_x_sym_B1(N,Mx) = 1.0D0
        ldu_x_sym_C1(N,Mx) = 0.0D0
       End Do
      endif
      ...
      if(yrank .EQ. 0)then
       Do N = 1,LotY
        ...
        ldu_y_sym_B1(N,1) = 1.0D0
        ldu_y_sym_C1(N,1) = 0.0D0
       End Do
      endif

      if(yrank .EQ. Py-1)then
       Do N = 1,LotY
        ldu_y_sym_A1(N,My) = 2.0D0
        ldu_y_sym_B1(N,My) = 1.0D0
        ldu_y_sym_C1(N,My) = 0.0D0
       End Do
      endif
      ...
      if(zrank .EQ. 0)then
       Do N = 1,LotZ
        ...
        ldu_z_sym_B1(N,1) = 1.0D0
        ldu_z_sym_C1(N,1) = 0.0D0
       End Do
      endif

      if(zrank .EQ. Pz-1)then
       Do N = 1,LotZ
        ldu_z_sym_A1(N,Mz) = 2.0D0
        ldu_z_sym_B1(N,Mz) = 1.0D0
        ldu_z_sym_C1(N,Mz) = 0.0D0
       End Do
      endif
...
*                                        18
      ...
      if(xrank .EQ. 0)then
       Do N = 1,LotX
        ldu_x_sym_A1(N,2) = alpha1_pade6
        ldu_x_sym_B1(N,2) = 1.0D0 
        ldu_x_sym_C1(N,2) = alpha1_pade6
       End Do
      endif

      if(xrank .EQ. Px-1)then
       Do N = 1,LotX
        ldu_x_sym_A1(N,Mx-1) = 1.0D0 / 2.0D0
        ldu_x_sym_B1(N,Mx-1) = 1.0D0
        ldu_x_sym_C1(N,Mx-1) = 1.0D0 / 6.0D0
       End Do 
      endif

      if(yrank .EQ. 0)then
       Do N = 1,LotY
        ldu_y_sym_A1(N,2) = alpha1_pade6 
        ldu_y_sym_B1(N,2) = 1.0D0 
        ldu_y_sym_C1(N,2) = alpha1_pade6
       End Do
      endif

      if(yrank .EQ. Py-1)then
       Do N = 1,LotY
        ldu_y_sym_A1(N,My-1) = 1.0D0 / 2.0D0
        ldu_y_sym_B1(N,My-1) = 1.0D0
        ldu_y_sym_C1(N,My-1) = 1.0D0 / 6.0D0
       End Do
      endif

      if(zrank .EQ. 0)then
       Do N = 1,LotZ
        ldu_z_sym_A1(N,2) = alpha1_pade6 
        ldu_z_sym_B1(N,2) = 1.0D0
        ldu_z_sym_C1(N,2) = alpha1_pade6
       End Do
      endif

      if(zrank .EQ. Pz-1)then
       Do N = 1,LotZ
        ldu_z_sym_A1(N,Mz-1) = 1.0D0 / 2.0D0
        ldu_z_sym_B1(N,Mz-1) = 1.0D0
        ldu_z_sym_C1(N,Mz-1) = 1.0D0 / 6.0D0
       End Do
      endif
...
*   beta =   3.8325242145930416 E-02
      ...
      Do N = 1, LotX
       Do  I = 1,Mx
        ldu_x_sym_A2(N,I) = alpha2_pade6
        ldu_x_sym_B2(N,I) =        1.0D0
        ldu_x_sym_C2(N,I) = alpha2_pade6
       End Do
      End Do

      Do N = 1,LotY
       Do J = 1, My
        ldu_y_sym_A2(N,J) = alpha2_pade6
        ldu_y_sym_B2(N,J) =        1.0D0
        ldu_y_sym_C2(N,J) = alpha2_pade6
       End Do
      End Do

      Do N = 1,LotZ
       Do K = 1, Mz
        ldu_z_sym_A2(N,K) = alpha2_pade6
        ldu_z_sym_B2(N,K) =        1.0D0
        ldu_z_sym_C2(N,K) = alpha2_pade6
       End Do
      End Do
...
*                               - f(N-3) ]
      if(xrank .EQ. 0)then
       Do N = 1,LotX
        ...
        ldu_x_sym_B2(N,1) = 1.0D0
        ldu_x_sym_C2(N,1) = 2D0 * alpha2_pade6 
       End Do
      endif

      if(xrank .EQ. Px-1)then
       Do N = 1,LotX
        ldu_x_sym_A2(N,Mx) = 1.1D1
        ldu_x_sym_B2(N,Mx) = 1.0D0
        ldu_x_sym_C2(N,Mx) = 0.0D0
       End Do
      endif
      ...
      if(yrank .EQ. 0)then
       Do N = 1,LotY
        ...
        ldu_y_sym_B2(N,1) = 1.0D0
        ldu_y_sym_C2(N,1) = 2D0 * alpha2_pade6 
       End Do
      endif

      if(yrank .EQ. Py-1)then
       Do N = 1,LotY
        ldu_y_sym_A2(N,My) = 1.1D1
        ldu_y_sym_B2(N,My) = 1.0D0
        ldu_y_sym_C2(N,My) = 0.0D0
       End Do
      endif
      ...
      if(zrank .EQ. 0)then
       Do N = 1,LotZ
        ...
        ldu_z_sym_B2(N,1) = 1.0D0
        ldu_z_sym_C2(N,1) = 2D0 * alpha2_pade6
       End Do
      endif

      if(zrank .EQ. Pz-1)then
       Do N = 1,LotZ
        ldu_z_sym_A2(N,Mz) = 1.1D1
        ldu_z_sym_B2(N,Mz) = 1.0D0
        ldu_z_sym_C2(N,Mz) = 0.0D0
       End Do
      endif
...
*                                           80
      if(xrank .EQ. 0)then
       Do N = 1,LotX
        ldu_x_sym_A2(N,2) = alpha2_pade6 
        ldu_x_sym_B2(N,2) = 1.0D0
        ldu_x_sym_C2(N,2) = alpha2_pade6
       End Do
      endif

      if(xrank .EQ. Px-1)then
       Do N = 1,LotX
        ldu_x_sym_A2(N,Mx-1) = - 7.0D0 / 2.0D1
        ldu_x_sym_B2(N,Mx-1) =   1.0D0
        ldu_x_sym_C2(N,Mx-1) =   1.0D-1
       End Do
      endif

      if(yrank .EQ. 0)then
       Do N = 1,LotY
        ldu_y_sym_A2(N,2) = alpha2_pade6
        ldu_y_sym_B2(N,2) = 1.0D0 
        ldu_y_sym_C2(N,2) = alpha2_pade6
       End Do
      endif

      if(yrank .EQ. Py-1)then
       Do N = 1,LotY
        ldu_y_sym_A2(N,My-1) = - 7.0D0 / 2.0D1
        ldu_y_sym_B2(N,My-1) =   1.0D0
        ldu_y_sym_C2(N,My-1) =   1.0D-1
       End Do
      endif

      if(zrank .EQ. 0)then
       Do N = 1,LotZ
        ldu_z_sym_A2(N,2) = alpha2_pade6
        ldu_z_sym_B2(N,2) = 1.0D0
        ldu_z_sym_C2(N,2) = alpha2_pade6
       End Do
      endif

      if(zrank .EQ. Pz-1)then 
       Do N = 1,LotZ
        ldu_z_sym_A2(N,Mz-1) = - 7.0D0 / 2.0D1
        ldu_z_sym_B2(N,Mz-1) =   1.0D0
        ldu_z_sym_C2(N,Mz-1) =   1.0D-1
       End Do
      endif
...
* Putting first 2 and last 2 data planes into temp arrays for sending
      temp(:,:,:,1) = F(:,:,1:2)
      temp(:,:,:,2) = F(:,:,Mx-1:Mx)
...
*  Solve the matrix system to obtain the derivative dF
      Call tridiagonal(ldu_x_sym_A1, ldu_x_sym_B1, ldu_x_sym_C1,
...
*  Wait to recieve planes
      ...
      Do J = 1, My
        Do K = 1, Mz
          ...
          if(xrank .EQ. 0)then
           W(N,1) = fac * ( b2_pade6 * 2D0 * ( F(J,K,3) - F(J,K,1) ) 
     .                    + a2_pade6 * 2D0 * ( F(J,K,2) - F(J,K,1) ) )
                    
           W(N,2) = fac * (b2_pade6*(F(J,K,4)-2D0*F(J,K,2) + F(J,K,2) ) 
...
*  Solve the matrix system to obtain the derivative dF
      Call tridiagonal(ldu_x_sym_A2, ldu_x_sym_B2, ldu_x_sym_C2,
     .                 W, Mx, LotX, 0, MPI_X_COMM)
...
*  Solve the matrix system to obtain the derivative dF
      Call tridiagonal(ldu_y_sym_A1, ldu_y_sym_B1, ldu_y_sym_C1,
...
*  Wait to recieve planes
      ...
      Do I = 1, Mx
        Do K = 1, Mz
          ...
          if(yrank .EQ. 0)then
           W(N,1) = fac * ( b2_pade6 * 2D0 * ( F(3,K,I) - F(1,K,I) ) 
     .                    + a2_pade6 * 2D0 * ( F(2,K,I) - F(1,K,I) ))

           W(N,2) = fac * (b2_pade6*(F(4,K,I)-2D0*F(2,K,I) + F(2,K,I) )
...
*  Solve the matrix system to obtain the derivative dF
      Call tridiagonal(ldu_y_sym_A2, ldu_y_sym_B2, ldu_y_sym_C2,
     .                W, My, LotY, 0, MPI_Y_COMM)
...
*  Solve the matrix system to obtain the derivative dF
      Call tridiagonal(ldu_z_sym_A1, ldu_z_sym_B1, ldu_z_sym_C1,
...
*  Wait to recieve planes
      ...
      Do I = 1, Mx
        Do J = 1, My
          ...
          if(zrank .EQ. 0)then
          W(N,1) = fac * ( b2_pade6 * 2D0 * ( F(J,3,I) - F(J,1,I) ) 
     .                   + a2_pade6 * 2D0 * ( F(J,2,I) - F(J,1,I) ))

          W(N,2) = fac * ( b2_pade6*(F(J,4,I)-2D0*F(J,2,I) + F(J,2,I) )
...
*  Solve the matrix system to obtain the derivative dF
      Call tridiagonal(ldu_z_sym_A2, ldu_z_sym_B2, ldu_z_sym_C2,
     .                 W, Mz, LotZ, 0, MPI_Z_COMM)
