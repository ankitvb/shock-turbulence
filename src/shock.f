...
! Add shock to primitive variables
     ...
     .         .AND.(abs(IZ).EQ.0.5D0)  )then
              P(J,K,I)  = P(J,K,I) + (gamma-1D0) * Q(J,K,I,4) * 1.5D0  ! 25 for 256 and 512, 2.0 for 128
     .                 * 0.1528415451D2 * exp(-rad*rad/(eps*eps)) ! Factor 4 for 512 grid, 1 for 256
     .                 / (eps * eps * eps)  
            elseif( (abs(IX).LT.2D0)
     .         .AND.(abs(IY).LT.2D0)
     .         .AND.(abs(IZ).LT.2D0)  )then
              P(J,K,I)  = P(J,K,I) + (gamma-1D0) * Q(J,K,I,4) * 1.5D0   ! 25 for 256 and 512, 2.0 for 128
     .                 * 0.1528415451D2 * exp(-rad*rad/(eps*eps)) ! Factor 2 for 512 grid, 1 for 256
     .                 / (eps * eps * eps)
            elseif( (abs(IX).LT.3D0)
     .         .AND.(abs(IY).LT.3D0)
     .         .AND.(abs(IZ).LT.3D0)  )then
              P(J,K,I)  = P(J,K,I) + (gamma-1D0) * Q(J,K,I,4) * 1.5D0   ! 25 for 256 and 512, 2.0 for 128
     .                 * 0.1528415451D2 * exp(-rad*rad/(eps*eps)) ! Factor 1.5 for 512 grid, 1 for 256
     .                 / (eps * eps * eps)
            else 
              P(J,K,I)  = P(J,K,I) + (gamma-1D0) * Q(J,K,I,4) * 1.5D0   ! 25 for 256 and 512, 2.0 for 128
     .                 * 0.1528415451D2 * exp(-rad*rad/(eps*eps)) 
     .                 / (eps * eps * eps)
            endif
