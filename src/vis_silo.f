...
***********************************************************************
      ...
      Parameter (num_groups = 256)  ! Number of output files (= # of I/O channels)
      Parameter (nfields = 10)       ! Number of output variables 
...
* Writing mesh and variable data to file           
      Do N = 0, group_size-1
        if(group_rank .EQ. N)then
          if(N .EQ. 0)then                                           ! group root creates group mesh file
           ierr = dbcreate_(groupfilename, lgroupfilename, DB_CLOBBER,
     .                     DB_LOCAL, "LMesh", 5, DB_PDB, dbfile)
            if(dbfile .EQ. -1) then
              print *,'Could not create silo file !'
            endif
          else
            ierr = dbopen_(groupfilename, lgroupfilename, DB_PDB, 
     .                    DB_APPEND, dbfile)                         ! every other process opens the group mesh file
          ...
          err  = dbmkdir_(dbfile, rank_str,5, ierr)                   ! each process creates and changes to its
          ierr = dbsetdir_(dbfile, rank_str, 5)                       ! own subdirectory within the file

          err  = dbputqm_(dbfile, "quadmesh", 8, "yc", 2,             ! each process writes to the group mesh file 
     .           "zc", 2, "xc", 2, yy, zz, xx, ldims,
     .           ndims, DB_DOUBLE, DB_COLLINEAR, DB_F77NULL, ierr) 

          err = dbputqv1_(dbfile, "rho", 3, "quadmesh", 8,          ! All processes write to group var file
     .          W(1,1,1,4),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "U", 1, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,1),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "V", 1, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,2),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "W", 1, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,3),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "P", 1, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,5),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "omx", 3, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,6),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "omy", 3, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,7),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "omz", 3, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,8),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "dil", 3, "quadmesh", 8,            ! All processes write to group var file
     .          W(1,1,1,9),  ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
          err = dbputqv1_(dbfile, "shock", 5, "quadmesh", 8,
     .          W(1,1,1,10), ldims, ndims, DB_F77NULL, 0,
     .          DB_DOUBLE, DB_NODECENT, DB_F77NULL, ierr)
 
          ierr = dbclose_(dbfile)
        endif
...
* Generating master file name
     ...
     .               '.silo'  
      ...
      if(rank .EQ. 0)then
        ierr = dbcreate_(rootfilename, lrootfilename, DB_CLOBBER,
...
* Saving step and simulation time as options
        ierr = dbmkoptlist_(2, optlistid)
        ierr = dbaddiopt_(optlistid, DBOPT_CYCLE, step)
        ierr = dbadddopt_(optlistid, DBOPT_DTIME, time)

* Set max string length
        oldlen = dbget2dstrlen_()
        err    = dbset2dstrlen_(128)

* Write the multimesh object
        err   = dbputmmesh_(dbfile, "quadmesh", 8, nmesh, meshfilenames,
...
* Write the multivar objects
     ...
     .                       TRIM(ADJUSTL(varname))
          ...
          err = dbputmvar_(dbfile, varname, lvarname, nvarfiles,
     .                      varfnames, lvarfnames, vartypes,
     .                      DB_F77NULL, ierr)
        End Do

* Cleaning up
        err = dbset2dstrlen_(oldlen)
        err = dbfreeoptlist_(optlistid)
...
* Process # 0 recieves particle data in order
     ...
     .                    M, MPI_COMM_WORLD, status, ierr)
      ...
      if(rank .EQ. 0)then
        ierr = dbcreate_(partfilename, lpartfilename, DB_CLOBBER,
     .                  DB_LOCAL,"Particle Data",13, DB_PDB, dbfile)
        ...
        ierr = dbputpm_(dbfile, "pointmesh", 9, ndims, partall(1,1),
     .                 partall(1,2), partall(1,3), Npartall, DB_DOUBLE,
     .                 optlistid, err)

        ierr = dbputpv1_(dbfile, "rho", 3, "pointmesh", 9,
     .               partall(1,4), Npartall, DB_DOUBLE, DB_F77NULL, err)
        ierr = dbputpv1_(dbfile, "P", 1, "pointmesh", 9,
     .               partall(1,5), Npartall, DB_DOUBLE, DB_F77NULL, err)
        ierr = dbputpv1_(dbfile, "om", 2, "pointmesh", 9,
     .               partall(1,6), Npartall, DB_DOUBLE, DB_F77NULL, err)
        ierr = dbputpv1_(dbfile, "dil", 3, "pointmesh", 9,
