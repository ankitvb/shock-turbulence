...
************************************************************************
      ...
      Real*8  Qf(My,Mz,Mx,5), Q_aux(My,Mz,Mx,5)
...
* Initializing particles if needed
      if(part_param.EQ.1) Call Init_Part

* Initializing shock if needed
!      Call Add_Shock_Expanding(Q)     ! Shock-turbulence
...
*  Open output file and write out problem parameters
     ...
     .                                                  'step = ', step
      ...
      Call Compute_Mass(Q); Call Compute_Energy(Q)
...
*  Compute time step subject to a constant CFL criterion
      ...
      Call Compute_Tstep(Q, cfl_resv, step)
...
c            Call Write_Restart(time, step)
