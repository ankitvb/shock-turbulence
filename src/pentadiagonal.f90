subroutine pentadiagonal(A1,B1,C1,D1,E1,R,n,lot,nper,ncom)
  implicit none
  ...
  ! Matrix
  Real*8, dimension(lot,n) :: A1,A     ! LOWER-2
  Real*8, dimension(lot,n) :: B1,B     ! LOWER-1
  Real*8, dimension(lot,n) :: C1,C     ! DIAGONAL
  Real*8, dimension(lot,n) :: D1,D     ! UPPER+1
  Real*8, dimension(lot,n) :: E1,E     ! UPPER+2
  Real*8, dimension(lot,n) :: R        ! RHS - RESULT
  ! Local
  Real*8, dimension(lot)     :: const
  Real*8, dimension(lot,2)   :: r1
  Real*8, dimension(lot,2)   :: r2
  Real*8, dimension(lot,n,2) :: s1
  Real*8, dimension(lot,n,2) :: s2
  ! Communication
  ...
  Real*8, dimension(:,:),   allocatable :: swap
  ! Stuff
  ...
  if (proc .eq. 1) then
     if (nper.eq.0) then
        ...
        call pentadiagonal_periodic_serial(A,B,C,D,E,R,n,lot)
     end if
     return
  end if
  ...
  if (n > 1) then ! do normal stuff
     ...
     s2(:,n,2) = e(:,n)
     if (nper .eq. 0) then
        if (rank .eq. 0)      s1(:,1:2,1:2)   = 0.0
        if (rank .eq. proc-1) s2(:,n-1:n,1:2) = 0.0
     end if
  ...
  do igroup=1,proc
     ...
     if (n > 1) then ! do normal stuff
        ...
        sendbuf(1:nk, L+2) = 1.0
        sendbuf(1:nk, L+3) = 0.0
        sendbuf(1:nk, L+4) = 1.0
        sendbuf(1:nk, L+5) = 0.0
        sendbuf(1:nk, L+6) = c(k1:k2, 1)
        sendbuf(1:nk, L+7) = r(k1:k2, 1)
        sendbuf(1:nk, L+8) = a(k1:k2, 1)
        sendbuf(1:nk, L+9) = d(k1:k2, 1)
        sendbuf(1:nk, L+10) = 0.0
        sendbuf(1:nk, L+11) = 0.0
        sendbuf(1:nk, L+12) = b(k1:k2, 1)
        sendbuf(1:nk, L+13) = e(k1:k2, 1)
        sendbuf(1:nk, L+14) = -1.0
        sendbuf(1:nk, L+15) = 0.0
        sendbuf(1:nk, L+16) = 0.0
        sendbuf(1:nk, L+17) = -1.0
        sendbuf(1:nk, L+18) = a(k1:k2, 1)
        sendbuf(1:nk, L+19) = d(k1:k2, 1)
        sendbuf(1:nk, L+20) = 0.0
        sendbuf(1:nk, L+21) = 0.0
  ...
  call MPI_AllToAll (sendbuf,nlot*24,MPI_REAL8,recvbuf,nlot*24,MPI_REAL8,ncom,ierr)
  ...
  call MPI_AllToAll(recvbuf,nlot*4,MPI_REAL8,sendbuf,nlot*4,MPI_REAL8,ncom,ierr)
...
!  use precision
  ...
  Real*8, dimension(lot,n) :: A     ! LOWER-2
  Real*8, dimension(lot,n) :: B     ! LOWER-1
  Real*8, dimension(lot,n) :: C     ! DIAGONAL
  Real*8, dimension(lot,n) :: D     ! UPPER+1
  Real*8, dimension(lot,n) :: E     ! UPPER+2
  Real*8, dimension(lot,n) :: R     ! RHS - RESULT
  Real*8, dimension(lot) :: const
  Integer :: i
  ...
  do i=1,n-2
     ! Eliminate A(2,i+1)
     const(:) = B(:,i+1)/(C(:,i)+tiny(1.0))
     C(:,i+1) = C(:,i+1) - D(:,i)*const(:)
     D(:,i+1) = D(:,i+1) - E(:,i)*const(:)
     R(:,i+1) = R(:,i+1) - R(:,i)*const(:)

     ! Eliminate A(1,i+2)
     const(:) = A(:,i+2)/(C(:,i)+tiny(1.0))
     B(:,i+2) = B(:,i+2) - D(:,i)*const(:)
     C(:,i+2) = C(:,i+2) - E(:,i)*const(:)
     R(:,i+2) = R(:,i+2) - R(:,i)*const(:)
  end do
  ! Eliminate A(2,n)
  const(:) = B(:,n)/(C(:,n-1)+tiny(1.0))
  C(:,n) = C(:,n) - D(:,n-1)*const(:)
  R(:,n) = R(:,n) - R(:,n-1)*const(:)
  
  ! Back-substitution
  R(:,n) = R(:,n)/(C(:,n)+tiny(1.0))
  R(:,n-1) = (R(:,n-1) - D(:,n-1)*R(:,n))/(C(:,n-1)+tiny(1.0))
  do i=n-2,1,-1
     R(:,i) = (R(:,i) - D(:,i)*R(:,i+1) - E(:,i)*R(:,i+2))/(C(:,i)+tiny(1.0))
  end do
...
! ============================================= !
subroutine pentadiagonal_periodic_serial(A,B,C,D,E,R,n,lot)
!  use precision
  implicit none
