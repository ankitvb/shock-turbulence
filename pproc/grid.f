...
*
*  x-grid:
      Call Uniform_X(Nx, x_leng, gxcor, x_start)
...
*  y_grid:
      Call Uniform_Y(Ny, y_leng, gycor, y_start)
...
*  z-grid:
      Call Uniform_Z(Nz, z_leng, gzcor, z_start)
...
************************************************************************
      Subroutine Uniform_X(Nx, x_leng, gxcor, x_start)

      implicit none
      Integer Nx
      Real*8  x_leng, gxcor(Nx), x_start
...
*  Generate x-mesh
      Do I = 1, Nx
        gxcor(I) = x_start + x_leng * Dble(I - 1) / Dble(Nx - 1)
      End Do
...
************************************************************************
      Subroutine Uniform_Y(Ny, y_leng, gycor, y_start)

      implicit none
      Integer Ny
      Real*8  y_leng, gycor(Ny), y_start
...
*  Generate y-mesh
      Do J = 1, Ny
        gycor(J)   = y_start + y_leng * Dble(J - 1) / Dble(Ny - 1)
      End Do
...
************************************************************************
      Subroutine Uniform_Z(Nz, z_leng, gzcor, z_start)

      implicit none
      Integer Nz
      Real*8  z_leng, gzcor(Nz), z_start
...
*  Generate z-mesh
      Do K = 1, Nz
        gzcor(K) = z_start + z_leng * Dble(K - 1) / Dble(Nz - 1)
      End Do
