...
*  Write out initial fields
      cue_vis = 1 
      If (out_init .EQ. 1) Call Output(time, step)
...
*  Write column headers
      If (rank .EQ. 0) Then
       OPEN(UNIT=out_unit,FILE="output",FORM="FORMATTED",
     .      STATUS="UNKNOWN", POSITION="APPEND")
       WRITE(out_unit,10) 'n ', 't   ', 
     .    'KE     ', 'MaxDil  ','Enstrophy ',
     .       'MomX  ', 'MomY  ','MomZ  ','Mass  ','Energy ','Entropy'
       WRITE(out_unit,11) step, time, KE, MaxDil,
...
************************************************************************
      If (march_mode .EQ. 1) Then
        Do step_count = 2, Nt_restart 
          Call Read_Restart(time, step) 
          cue_restart = cue_restart + 1  

*  Write out flow field data 
          if(step.NE.0)then
...
c      Call Write_Visual_Silo(time, step)
