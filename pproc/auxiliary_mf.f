...
************************************************************************
c      Subroutine Compute_Gradient(F, Fgrad)
c 
c      Include 'header'
c
c      Real*8 F(My,Mz,Mx), Fgrad(My,Mz,Mx,3)
c      Integer I, J, K, L
c     
c      Call DFDX_sym(F, Fgrad(1,1,1,1), 0, 1D0)
c      Call DFDY_sym(F, Fgrad(1,1,1,2), 0, 1D0)
c      Call DFDZ_sym(F, Fgrad(1,1,1,3), 0, 1D0)
c   
c      End Subroutine Compute_Gradient
************************************************************************
************************************************************************
* Subroutine: Compute_PDF
*
* Computes the PDF for a given field 
* Divides range of the data into N_pdf bins
*
* In: F: array with dimensions my, mz, mx
*     mx, ny, nz: array dimensions
*
* Out: F_pdf: PDF of F
* F_pdf(:,1) gives values of F (x-axis)
* F_pdf(:,2) gives probability density
*
************************************************************************
      Subroutine Compute_PDF(F, F_pdf)

      Include 'header'
      Include 'mpif.h'

*  Procedure parameters
      Real*8  F(My,Mz,Mx), F_pdf(N_pdf,2)
      Real*8  F_min, F_max, dF, mix_flag
...
*  Locals
      Integer I, J, K, M, N
      Integer count_local(N_pdf), count_global(N_pdf)
      Integer sum_count_global

      Call Compute_Min(F, F_min)
      Call Compute_Max(F, F_max) 

      F_pdf = 0D0       

! Setting data range
      dF = (F_max - F_min) / DBLE(N_pdf - 1)
      F_pdf(1,1) = F_min     

      Do I = 2,N_pdf
        F_pdf(I,1) = F_pdf(I-1,1) + dF
      End Do
...
! Scanning and binning data in the mixing layer
! mix_flag is a flag that detects whether point is in the mixing layer or not
! This flag is a sensor Y(1-Y) which decays sharply at the edges of the mixing layer
! and hence a reliable indicator
      Do I = 1,Mx
       Do J = 1,My
        Do K = 1,Mz
          mix_flag = Y(J,K,I,1) * (1D0 - Y(J,K,I,1))
          if(mix_flag .GE. 0.01D0)then
            Do M = 1,N_pdf-1
             if((F(J,K,I).GE.F_pdf(M,1)).AND.
     .          (F(J,K,I).LT.F_pdf(M+1,1)))then
              count_local(M) = count_local(M) + 1
             endif
            End Do 
          endif                 
...
! Summing up over all processors for each bin
      Call MPI_ALLREDUCE(count_local, count_global, N_pdf, MPI_INTEGER,
     .                   MPI_SUM, MPI_COMM_WORLD, ierr)
 
      sum_count_global = SUM(count_global)

! Computing PDF 
      Do M = 1,N_pdf
        F_pdf(M,2) = DBLE(count_global(M)) / (dF*DBLE(sum_count_global))
      End Do
