...
*  Flow parameters
      Call MPI_BCAST(Re1,        1, MPI_REAL8,   0, MPI_COMM_WORLD, 
     .               ierr)
      Call MPI_BCAST(Re2,        1, MPI_REAL8,   0, MPI_COMM_WORLD,
     .               ierr)
...
*  Define inverse of Reynolds number (allow inviscid calculation)
      If (Re1 .GT. 0.0D0) Then
        Re1_inv = 1.0D0 / Re1
      Else
        Re1_inv = 0.0D0
      End If
    
      If (Re2 .GT. 0.0D0) Then
        Re2_inv = 1.0D0 / Re2
      Else
        Re2_inv = 0.0D0
      End If
...
*  Locals
     ...
     .      STATUS="UNKNOWN", POSITION="APPEND")
      ...
      Write(out_unit,20) 'Re1        = ', Re1
      Write(out_unit,20) 'Re2        = ', Re2
